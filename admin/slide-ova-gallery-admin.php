...
function slide_ova_video_box() {
  ...
  $html =  '<input type="hidden" name="slide_video_noncename" value="' . wp_create_nonce(plugin_basename(__FILE__)) . '" />';
...
function slide_ova_save_data($post_id, $post) {
  ...
  if ( !isset($_POST['slide_gallery_noncename']) || !wp_verify_nonce($_POST['slide_gallery_noncename'], plugin_basename(__FILE__) )) {
    return $post->ID;
  }
  ...
  // $mydata['slide-ova-img-url'] = $_POST['slide_ova_image_url'];
  global $wpdb;
